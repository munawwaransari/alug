...
class posAPI {
	...
	addNounInfoHtml(container, res){
		...
		if (filters.length > 0) {
			container.prepend($(getListButtinWithSelect(`
				<select class="nFilter" onchange="filterTableRows(4, '#nTable', 4, $('.nFilter').val().replace(' - ',''))">
		...
		if(filters.length > 0){
			var sel = `
				<select class="nFilter"
							onchange="filterTableRows(3, '#pTable', 3, $('.nFilter').val().replace(' - ',''))">
						<option value="all">Show All</option>
						${
							filters.map((n) => {
	...
	addPrepPhrasesInfoHtml(container, res, title){
		...
		if(filters.length > 0){
			var sel = `
				<select class="nFilter" 
					  	onchange="filterTableRows(-1, '#pTable', -1, $('.nFilter').val(), {useInclude: true})">
					  	<option value="all">Show All</option>
						${
							filters.map((n) => {
								var keys =  n.split("|");	
